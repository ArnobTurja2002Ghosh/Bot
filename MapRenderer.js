			...
			**/
			...
			switch (node.type) {
				case (TileNode.Type.SomethingElse):
					...
					gameObject.add(mesh);
					break;
				case (TileNode.Type.Garbage):
					material = new THREE.MeshStandardMaterial({ color: 0x00ff00 });
					mesh = new THREE.Mesh(geometry, material);

					gameObject.add(mesh);
					break;
				case (TileNode.Type.TB):
					material = new THREE.MeshStandardMaterial({ color: 0x005500 });
					mesh = new THREE.Mesh(geometry, material);

					gameObject.add(mesh);
					break;
				case (TileNode.Type.Recyclable):
					material = new THREE.MeshStandardMaterial({ color: 0x0000ff });
					mesh = new THREE.Mesh(geometry, material);

					gameObject.add(mesh);
					break;	
				case (TileNode.Type.RB):
					material = new THREE.MeshStandardMaterial({ color: 0x000055 });
					mesh = new THREE.Mesh(geometry, material);

					gameObject.add(mesh);
					break;	
				case (TileNode.Type.Charger):
					material = new THREE.MeshStandardMaterial({ color: 0xff0000 });
					mesh = new THREE.Mesh(geometry, material);
